...
class Env(tk.Tk):
    ...
    def load_images(self):
        ...
        triangle = PhotoImage(Image.open("../../Images/triangle.png").resize((30, 30)))
        circle = PhotoImage(Image.open("../../Images/circle.png").resize((30, 30)))
    ...
    def set_reward(self, state, reward):
        ...
        if reward > 0:
            ...
            temp["figure"] = self.canvas.create_image(
                (UNIT * x) + UNIT / 2, (UNIT * y) + UNIT / 2, image=self.shapes[2]
            )
            ...
            temp["reward"] = reward
            temp["figure"] = self.canvas.create_image(
                (UNIT * x) + UNIT / 2, (UNIT * y) + UNIT / 2, image=self.shapes[1]
            )
    ...
    def move_const(self, target):
        ...
        if target["figure"] is not self.rectangle and s == [
            (WIDTH - 1) * UNIT,
            (HEIGHT - 1) * UNIT,
        ]:
            base_action = np.array([0, 0])
